<template>
  <div class="player">
     ...
     <!-- <object data="http://127.0.0.1:5500/cn/demo.html" width="1000px" height="1000px"></object> -->
        ...
        <div id="videoWin" class="video" ></div>
  ...
  export default {
       ...
       window.I_InitPlugin({ 
        ...
        iWndowType: that.windowNum, // 改成2为4屏
          cbInitPluginComplete: function () {
            window.I_InsertOBJECTPlugin("videoWin").then(
              () => {
                  // window.I_Resize(30,30);
              //  debugger
             //   alert("初始化成功");
             console.info("初始化成功");
                // 检查插件是否最新
   ...
   console.info(2)
      ...
      //  window.I_Login("192.168.3.2", 1, "80", "admin", "admin12345", {
          ...
          //    console.info("已登录过");   
              this.realLogout(this.camInfo[0].camIp, this.camInfo[0].camPort);
         //   } else {
              // showOPInfo(
      ...
      realLogout(n_ip, n_port) {
        const that = this;
        for (var i = 0; i<that.camInfo.length; i++) {
          if (that.camInfo.length != 1) {
      //  window.I_Logout(this.szDeviceIdentify).then(
        window.I_Logout(that.camInfo[i].camIp + "_" + that.camInfo[i].camPort).then(
          ...
          //   showOPInfo(szDeviceIdentify + " " + "退出失败！");
            ...
            window.I_Logout(n_ip + "_" + n_port).then(
          () => {
          //   showOPInfo(szDeviceIdentify + " " + "退出成功！");
              console.info("退出成功");
          },
    ...
    mounted() {
     console.info("当前预览", this.camInfo);  
